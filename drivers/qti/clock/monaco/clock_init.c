 ...
 *
 * TF-A clock driver back-end for monaco (qcs8300): image init hooks and the
...
static int clock_enable_gpu_gx_gdsc(void)
{
	...
	while (((mmio_read_32(GPUCC_GPU_CC_GX_GDSCR) &
		 GPUCC_GPU_CC_GX_GDSCR_PWR_ON_BMSK) == 0U) &&
	       (--retry > 0U)) {
		udelay(CLOCK_STEP_DELAY_US);
	}
...
static int clock_enable_gpu_cx_gdsc(void)
{
	...
	while (((mmio_read_32(GPUCC_GPU_CC_CX_GDSCR) &
		 GPUCC_GPU_CC_CX_GDSCR_PWR_ON_BMSK) == 0U) &&
	       (--retry > 0U)) {
		udelay(CLOCK_STEP_DELAY_US);
	}
...
int clock_post_init_image(struct clock_drv_ctxt *drv_ctxt)
{
	...
	mmio_write_32(CL0_GOLDPLUS_PLL_SECURE, 0x0U);
	mmio_write_32(CL0_L3_PLL_SECURE, 0x0U);
	mmio_write_32(CL1_L3_PLL_SECURE, 0x0U);
	mmio_write_32(CL1_SILVER_PLL_SECURE, 0x0U);
